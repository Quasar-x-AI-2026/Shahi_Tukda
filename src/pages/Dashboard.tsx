...
const Dashboard = () => {
  ...
  const handleStartAnalysis = (file: File) => {
    ...
    // Clear previous analysis to ensure we don't show stale data if this one fails
    localStorage.removeItem("latestAnalysis");
  };

  const handleAnalysisComplete = (analysisData: any) => {
    console.log("📥 Dashboard: Received analysis data:", analysisData);
    console.log("🎯 Risk Score:", analysisData?.riskScore);

    setShowAnalysis(false);

    // Save analysis data to localStorage so ContractReport can access it
    if (analysisData) {
      ...
      console.log("⚠️ Dashboard: No analysis data to save!");
    }
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {actionCards.map((card, index) => (
                <motion.button
                  ...
                  className={`group relative p-6 rounded-2xl text-left transition-all overflow-hidden ${card.primary
                ...
                >
                  ...
                  <div className="relative z-10">
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 ${card.primary
                      ? "bg-white/20"
                      : "bg-muted"
                      }`}>
                      <card.icon className={`w-6 h-6 ${card.primary ? "text-white" : "text-muted-foreground"
                        }`} />
                    </div>
                    <h3 className={`text-lg font-semibold mb-2 ${card.primary ? "text-white" : "text-foreground"
                      }`}>
                      {card.title}
                    </h3>
                    <p className={`text-sm mb-4 ${card.primary ? "text-white/80" : "text-muted-foreground"
                      }`}>
                      {card.description}
                    </p>
                    <div className={`inline-flex items-center gap-2 text-sm font-medium ${card.primary ? "text-white" : "text-accent"
                      }`}>
                      Get Started
          ...
          >
            ...
            <div className="bg-card rounded-2xl border border-border/50 overflow-hidden">
              {recentActivity.map((activity, index) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${activity.riskLevel === "high" ? "bg-risk-high/10" :
                      activity.riskLevel === "medium" ? "bg-risk-medium/10" :
                        "bg-risk-low/10"
                      }`}>
                      <FileText className={`w-5 h-5 ${activity.riskLevel === "high" ? "text-risk-high" :
                  ...
                  <div className={`px-3 py-1 rounded-full text-xs font-medium ${activity.riskLevel === "high" ? "bg-risk-high/10 text-risk-high" :
