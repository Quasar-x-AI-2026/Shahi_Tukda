...
const AIAnalysisExperience = ({ file, onComplete }: AIAnalysisExperienceProps) => {
  ...
  // Use ref to store the analysis result so it's always current when callback is called
  ...
  useEffect(() => {
    ...
    const uploadAndAnalyze = async () => {
      try {
        ...
        console.log("📤 AIAnalysisExperience: Uploading file to backend...");
        ...
        console.log("✅ AIAnalysisExperience: Backend Analysis Result:", result);
        console.log("🎯 Risk Score from backend:", result.riskScore);
        console.log("📊 Total risks:", result.risks?.length);

        // Store in both state and ref
        setAnalysisResult(result);
        analysisResultRef.current = result;
        console.log("💾 Stored analysis result in ref:", analysisResultRef.current);
      } catch (error) {
        console.error("❌ AIAnalysisExperience: Upload failed:", error);
      }
    };
    ...
    // Continue with animation timing
      ...
      setTimeout(() => {
        ...
        if (index === analysisSteps.length - 1) {
          setTimeout(() => {
            setIsComplete(true);
            setTimeout(() => {
              console.log("🎬 AIAnalysisExperience: Animation complete, passing data to onComplete");
              console.log("📊 Analysis result from ref:", analysisResultRef.current);
        ...
        >
          {analysisSteps.map((step) => {
            ...
            return (
              <motion.div
                key={step.id}
                className={`relative p-3 rounded-xl border transition-all duration-300 ${isCompleted
              ...
              >
                <div className="flex items-center gap-2">
                  <div className={`w-6 h-6 rounded-lg flex items-center justify-center ${isCompleted
                    ? "bg-risk-low/20"
                    : isCurrent
                      ? "bg-accent/20"
